...
class TestRayS3DataLoadStrategy(DataJuicerTestCaseBase):
    ...
    def test_load_json_uses_read_json_stream(self):
        raw_dataset = object()
        wrapped_dataset = object()
        s3_fs = object()

        fake_ray, ray_modules, fake_ray_modules = build_fake_package("ray", "data")
        ray_modules["data"].read_parquet = MagicMock()
        ray_modules["data"].read_csv = MagicMock()
        ray_modules["data"].read_text = MagicMock()
        ray_modules["data"].read_numpy = MagicMock()
        ray_modules["data"].read_tfrecords = MagicMock()
        ray_modules["data"].read_lance = MagicMock()

        mock_read_json_stream = MagicMock(return_value=raw_dataset)
        mock_ray_dataset = MagicMock(return_value=wrapped_dataset)
        fake_ray_dataset_module = build_fake_ray_dataset_module(
            ray_dataset_cls=mock_ray_dataset,
            read_json_stream=mock_read_json_stream,
        )
        ...
        with (
            ...
            patch(
                "data_juicer.core.data.load_strategy.create_pyarrow_s3_filesystem",
                return_value=s3_fs,
            ),
        ):
            result = strategy.load_data()

        mock_read_json_stream.assert_called_once()
        call_args, call_kwargs = mock_read_json_stream.call_args
        self.assertEqual(call_args, ("s3://bucket-name/path/to/file.jsonl",))
        self.assertIs(call_kwargs["filesystem"], s3_fs)
        if "read_options" in call_kwargs:
            self.assertIsNone(call_kwargs["read_options"])
        mock_ray_dataset.assert_called_once_with(
            raw_dataset,
            dataset_path="s3://bucket-name/path/to/file.jsonl",
            cfg=self.cfg,
        )
        self.assertIs(result, wrapped_dataset)

    def test_load_data_uses_explicit_format_name(self):
        raw_dataset = object()
        wrapped_dataset = object()
        s3_fs = object()
        ...
        fake_ray, ray_modules, fake_ray_modules = build_fake_package("ray", "data")
        ray_modules["data"].read_parquet = MagicMock()
        ray_modules["data"].read_csv = MagicMock(return_value=raw_dataset)
        ray_modules["data"].read_text = MagicMock()
        ray_modules["data"].read_numpy = MagicMock()
        ray_modules["data"].read_tfrecords = MagicMock()
        ray_modules["data"].read_lance = MagicMock()
        ...
        fake_ray_dataset_module = build_fake_ray_dataset_module(
            ray_dataset_cls=mock_ray_dataset,
            read_json_stream=MagicMock(),
        )

        ds_config = {
            "type": "remote",
            "source": "s3",
            "path": "s3://bucket-name/path/to/file.anything",
            "format": "csv",
        }
        strategy = RayS3DataLoadStrategy(ds_config, self.cfg)
        ...
        ray_modules["data"].read_csv.assert_called_once_with(
            "s3://bucket-name/path/to/file.anything",
            filesystem=s3_fs,
        )
        mock_ray_dataset.assert_called_once_with(
            raw_dataset,
            dataset_path="s3://bucket-name/path/to/file.anything",
            cfg=self.cfg,
        )
        self.assertIs(result, wrapped_dataset)

    def test_load_data_defaults_to_parquet_for_unknown_extension(self):
        raw_dataset = object()
        wrapped_dataset = object()
        s3_fs = object()

        fake_ray, ray_modules, fake_ray_modules = build_fake_package("ray", "data")
        ray_modules["data"].read_parquet = MagicMock(return_value=raw_dataset)
        ray_modules["data"].read_csv = MagicMock()
        ray_modules["data"].read_text = MagicMock()
        ray_modules["data"].read_numpy = MagicMock()
        ray_modules["data"].read_tfrecords = MagicMock()
        ...
        ds_config = {
            "type": "remote",
            "source": "s3",
            "path": "s3://bucket-name/path/to/table",
        }
        strategy = RayS3DataLoadStrategy(ds_config, self.cfg)
        ...
        ray_modules["data"].read_parquet.assert_called_once_with(
            "s3://bucket-name/path/to/table",
            filesystem=s3_fs,
        )
        mock_ray_dataset.assert_called_once_with(
            raw_dataset,
            dataset_path="s3://bucket-name/path/to/table",
            cfg=self.cfg,
        )
        self.assertIs(result, wrapped_dataset)
    ...
    def test_load_data_wraps_reader_errors(self):
        ...
        fake_ray, ray_modules, fake_ray_modules = build_fake_package("ray", "data")
        ray_modules["data"].read_parquet = MagicMock(side_effect=ValueError("parquet broken"))
        ray_modules["data"].read_csv = MagicMock()
        ray_modules["data"].read_text = MagicMock()
        ray_modules["data"].read_numpy = MagicMock()
        ray_modules["data"].read_tfrecords = MagicMock()
        ...
        self.assertIn("Failed to load parquet data from S3 path s3://bucket-name/path/to/table", str(ctx.exception))
        self.assertIn("parquet broken", str(ctx.exception))
...
class TestRayHDFSDataLoadStrategy(DataJuicerTestCaseBase):
    ...
    def test_load_json_uses_read_json_stream(self):
        ...
        with (
            ...
            patch.object(strategy, "_create_hdfs_fs", return_value=hdfs_fs),
        ):
            result = strategy.load_data()

        mock_read_json_stream.assert_called_once()
        call_args, call_kwargs = mock_read_json_stream.call_args
        self.assertEqual(call_args, ("hdfs://namenode:9000/data/sample.jsonl",))
        self.assertIs(call_kwargs["filesystem"], hdfs_fs)
        if "read_options" in call_kwargs:
            self.assertIsNone(call_kwargs["read_options"])
        mock_ray_dataset.assert_called_once_with(
            raw_dataset,
            dataset_path="hdfs://namenode:9000/data/sample.jsonl",
            cfg=self.cfg,
        )
        self.assertIs(result, wrapped_dataset)

    def test_load_csv_uses_ray_reader_from_extension(self):
        raw_dataset = object()
        wrapped_dataset = object()
        hdfs_fs = object()
        ...
        mock_ray_dataset.assert_called_once_with(
            raw_dataset,
            dataset_path="hdfs://namenode:9000/data/sample.csv",
            cfg=self.cfg,
        )
        self.assertIs(result, wrapped_dataset)
